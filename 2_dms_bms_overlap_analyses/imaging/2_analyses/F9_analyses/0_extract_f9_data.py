...
OUTPUT_DIR = Path("../../3_outputs/data/F9")
INPUT_VARCHAMP = Path("../../../../3_integrated_assay_analyses/1_inputs/VarChAMP_data_supp_mat_PP.tsv")

# F9 batch assignments (all F9 variants are in batch 13-14)
F9_BATCH_IDS = ("2025_01_27_Batch_13", "2025_01_28_Batch_14")


def load_varchamp_summary() -> pl.DataFrame:
    """Load VarChAMP summary and transform to expected format."""
    df = pl.read_csv(INPUT_VARCHAMP, separator="\t", infer_schema_length=100000)

    # Transform to match expected column names
    df = df.with_columns([
        pl.col("symbol").alias("Gene"),
        (pl.col("symbol") + "_" + pl.col("aa_change")).alias("gene_variant"),
        pl.col("imaging_score").alias("img_pheno_score"),
    ])
...
def extract_feature_importance(imaging_summary: pl.DataFrame) -> pl.DataFrame:
    ...
    # Get F9 variant list for filtering
    f9_summary = imaging_summary.filter(pl.col("Gene") == GENE)
    batch_allele_list = list(f9_summary["gene_variant"]) + [GENE]

    print("Loading feature importance data from batches...")
    for batch_id in F9_BATCH_IDS:
...
def compute_zscore_variant_features(
    ...
    metadata_cols = ["gene_variant", "img_pheno_score",
...
def main():
    ...
    # Load VarChAMP summary
    print("Loading VarChAMP summary...")
    imaging_summary = load_varchamp_summary()
    ...
    if not args.skip_features:
        ...
        nuc2cyto_df = compute_nuc2cyto_ratio(F9_BATCH_IDS)
